...
TOKEN = '7984256855:AAF6b5Td5CMeHRrB-RRAu2Zzu-OAyqci9pQ'
...
def check_expired_users():
    for user in users_collection.find():
        # Ensure the document is a dictionary and contains the 'user_id' key
        if not isinstance(user, dict) or 'user_id' not in user:
            print(f"Skipping invalid or malformed user document: {user}")
            continue

        # Extract user details safely
        user_id = user['user_id']
        expiry_date = user.get('expiry_date')  # Use .get() to avoid KeyError

        # Validate the expiry_date and check if the user has expired
        if expiry_date and isinstance(expiry_date, datetime):
            if datetime.now() >= expiry_date:
                # Mark user as expired in the database
                users_collection.update_one(
                    {"user_id": user_id}, {"$set": {"status": "expired"}}
                )
                print(f"User {user_id} has been expired.")
        else:
            print(f"User {user_id} has no valid expiry_date: {expiry_date}")
...
def download_file(message):
    ...
    try:
        ...
        if len(command_parts) < 2:
            bot.reply_to(message, "Please specify the file name. Example: /download filename.txt")
            return
...
def delete_key(message):
    ...
    # Parse the command to extract the key
    try:
        _, key = message.text.split()
    except ValueError:
    ...
    # Check if the key exists in the keys_collection
    key_data = keys_collection.find_one({"key": key})
    if not key_data:
        bot.reply_to(message, "❌ Key not found in the database.")
        return

    # Delete the key from the keys_collection
    keys_collection.delete_one({"key": key})

    bot.reply_to(message, f"✅ Key '{key}' has been successfully deleted.")
...
def attack_command(message):
    ...
    try:
        ...
        # Check if user exists and has an active plan
        ...
        if not valid_until or datetime.fromisoformat(valid_until) < now:
            ...
            bot.send_message(
                chat_id,
                "*Your approval has expired. \nPlease contact @shriram4311 to renew.*",
                parse_mode='Markdown',
                reply_markup=create_inline_keyboard()
            )
            return

        # Proceed with the attack if the user is valid
        bot.send_message(
            chat_id,
            "*Enter the target IP, port, and duration (in seconds) separated by spaces. \nE.g. - 167.67.25 69696 60*",
            parse_mode='Markdown',
            reply_markup=create_inline_keyboard()
        )
...
def canary_command(message):
    ...
    button1 = InlineKeyboardButton(
        ...
        url="https://t.me/filesharingbotbybot?start=BQADAQAD3hIAAtsuwEc0r-KJCttrHxYE")
...
10. /download - Download Files From Vps 
11. /delete - Deletes Generated Keys
